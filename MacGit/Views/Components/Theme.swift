...
/// Sémantické barevné tokeny značky Revision.
///
/// Každý token se překlápí sám podle vzhledu systému (dynamická `NSColor`), takže
/// v pohledech nezůstává žádné větvení `if scheme == .dark`. Syrová paleta je v `Brand`.
enum Theme {
    ...
    /// Akce, výběr, akcent aplikace. V tmavém režimu zesvětlený kvůli kontrastu proti navy.
    ...
    /// Akcent jako výplň pod bílým textem (tlačítka, odznaky). Zesvětlený akcent by bílý text neunesl.
    ...
    /// Tónování skla u vybraného řádku – ve světlém režimu jemnější, aby tmavý text držel kontrast.
    static let selectionTint = dynamic(light: 0x197BFF, lightAlpha: 0.16, dark: 0x4C9DFF, darkAlpha: 0.24)

    /// Gradient hlavních akcí (tlačítko commitu, AI jiskřička, vybraná kapsle).
    ...
    private static func dynamic(light: UInt32, lightAlpha: Double = 1, dark: UInt32, darkAlpha: Double = 1) -> Color {
